...
var InternalProps = ['message', 'required', 'forEach', 'forOwn', 'sanitize'];
...
Validator.prototype.build = function(data, where, options, parentDataObj, parentProp, parentTreeObj) {
    ...
    Object.keys(where).forEach(function(prop) {
        ...
        if (assertions.hasOwnProperty(prop)) {//builds single assertion
            ...
            var fun = self.buildArrayItemCondition(prefixedProp, data, where, options, parentTreeObj);
            if (fun instanceof Function) {
                pool.push(fun);
            }
        } else if (prop === 'forOwn') {// yields new pool of assertions for every item of an array
            var fun = self.buildMapItemCondition(prefixedProp, data, where, options, parentTreeObj);
            if (fun instanceof Function) {
                pool.push(fun);
            }
 ...
 * @return {Function}
 */
Validator.prototype.buildArrayItemCondition = function(prop, data, where, options, parentTreeObj) {
    var andCondPool = [];
    var keywordPrefix = options.keywordPrefix;
    var requiredKeyword = keywordPrefix + 'required';
    var messageKeyword = keywordPrefix + 'message';
    ...
    if ( data instanceof Array ) {
        var path = options.propPath ? options.propPath + '.' : '';
...
/*
 * buildMapItemCondition
 *
 * @param {string} prop = '$forOwn'
 * @param {object} data
 * @param {object} where - filter object
 * @param {object} [options]
 * @param {string} [options.propPath] - path to current property being validated. In dot notation
 * @return {Function}
 */
Validator.prototype.buildMapItemCondition = function(prop, data, where, options, parentTreeObj) {
    var andCondPool = [];
    var keywordPrefix = options.keywordPrefix;
    var requiredKeyword = keywordPrefix + 'required';
    var messageKeyword = keywordPrefix + 'message';
    ...
    andCondPool.push( this.buildAssertion(data, Object, 'is', {
        propPath: options.propPath,
        keywordPrefix: options.keywordPrefix,
        message: where[messageKeyword] || options.message
    }));
    ...
    if ( data instanceof Object ) {
        ...
        for (var i = 0, len = keys.length; i < len; i++) {
            var opt = _.assign({}, options, {
                comparisonOperator: options.comparisonOperator,
                required: required,
                propPath: path + '[' + keys[i] + ']',
                message: where[prop][messageKeyword] || where[messageKeyword] || options.message
            });
            andCondPool.push(this.build(data[keys[i]], where[prop], opt, data, keys[i], parentTreeObj));
        }
    }
